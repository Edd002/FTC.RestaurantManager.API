...
public class DateDeserializer extends JsonDeserializer<Date> {
    ...
    public Date deserialize(JsonParser jsonparser, DeserializationContext deserializationcontext) throws IOException {
        SimpleDateFormat dateFormat = DateUtil.configureDateFormat(DatePatternEnum.DATE_FORMAT_dd_mm_yyyy_WITH_SLASH.getValue());
        ...
        if (informedStringDate != null && !informedStringDate.isEmpty()) {
            try {
                ...
                if (!DateUtil.betweenEqual(informedFormattedDate, dateFormat.parse(DATE_LIMIT_BEGIN), dateFormat.parse(DATE_LIMIT_END))) {
